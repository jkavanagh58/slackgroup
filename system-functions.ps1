...
<#
	.SYNOPSIS
		Quick function to unlock a user account.
	.DESCRIPTION
		A detailed description of the Unlock-useraccount function.
	.PARAMETER userlogon
...
Begin {
	...
	Try { $usracct = Get-ADUser -Identity $userlogon -Properties LockedOut }
	Catch { "No user found with that samAccountName"; Exit}
}
Process {
	If ($usracct.LockedOut) {
		"Account is locked out"
		Try {
			Unlock-ADAccount -Identity $usracct
			"Account unlocked"
		}
		Catch {
			"Unable to unlock account, verify you have admin privileges"
		}
	}
