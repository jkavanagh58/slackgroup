...
.NOTES
    ...
    04.11.2017 JJK: With .net need to be able to detect special characters and handle with regex
#>
...
If (Test-path $configFldr){
    ...
    Foreach ($cFile in $configFiles){
        try {
            $reader = [System.IO.StreamReader] $cFile.FullName
            $data = $reader.ReadToEnd()
            if( $data.Contains($curVal)){"`t{0} will be updated" -f $cfile.Name}
            $reader.close()
        }
        finally {
            if ($reader -ne $null){
        ...
        $data = $data -replace $curVal, $newVal
        try {
            $writer = [System.IO.StreamWriter] $cFile.FullName
            $writer.write($data)
            $writer.close()
        }
        finally {
